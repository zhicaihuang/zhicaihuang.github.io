...
The filtering operation has complexity $\mathcal{O}(|\mathcal{E}| FC)$. as $\tilde{A} X$ can be efficiently implemented as a product of a sparse matrix with a dense matrix.
